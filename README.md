...
- **Deux morts bien différentes** : une mort normale s'éteint dans le noir (couleurs qui ternissent, iris qui se ferme) ; une mort spéciale fait briller Finn pendant que la scène s'assombrit (ralenti, halo doré, éclair, rayons courts, petites ondes, étincelles qui montent, son dédié), sans envahir l'écran. L'écran qui suit affiche le nouvel âge de Finn et une petite pique tirée au hasard (liste `TAUNTS` dans `views/death_view.py`, jamais deux fois la même d'affilée), en gris sur noir, ou en or pâle dans une pénombre chaude (halo doré qui respire, quelques braises) pour la mort spéciale.
